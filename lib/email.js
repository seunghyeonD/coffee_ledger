...
// context: { companyId, type } — email_logs 기록용
export async function sendNotificationEmails(emails, title, body, context = {}) {
  if (!emails || emails.length === 0) return 0;

  if (!isEmailConfigured()) {
    console.error('Email skipped: RESEND_API_KEY is not configured');
    await logEmails(emails, title, false, 'RESEND_API_KEY not configured', context);
    return 0;
  }
  ...
  for (let i = 0; i < emails.length; i += BATCH_SIZE) {
    const recipients = emails.slice(i, i + BATCH_SIZE);
    ...
    try {
      ...
      if (res.ok) {
        sent += batch.length;
        await logEmails(recipients, title, true, null, context);
      } else {
        const errText = await res.text();
        console.error('Resend batch error:', res.status, errText);
        await logEmails(recipients, title, false, `${res.status} ${errText}`.slice(0, 500), context);
      }
    } catch (e) {
      console.error('Resend request failed:', e);
      await logEmails(recipients, title, false, String(e?.message || e).slice(0, 500), context);
    }
  }
  return sent;
}

// 발송 이력 기록 — 테이블이 없거나 실패해도 발송 흐름에 영향 주지 않음
async function logEmails(recipients, subject, success, error, context) {
  try {
    const supabase = getSupabaseAdmin();
  ...
  <div style="max-width:480px;margin:0 auto;background:#ffffff;border-radius:12px;overflow:hidden;border:1px solid #e8e0d8;">
    ...
    <div style="padding:16px 24px;border-top:1px solid #f0ebe5;">
      <p style="margin:0;font-size:12px;color:#9b9187;">
        ...
        수신을 원치 않으시면 설정 &gt; 알림에서 이메일 알림을 끄거나, 계정이 없는 경우 관리자에게 요청해주세요.
      </p>
    </div>
  </div>
</div>`;
