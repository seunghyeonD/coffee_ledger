...
export function StoreProvider({ children }) {
  ...
  const init = useCallback(async () => {
    const [membersRes, shopsRes, menusRes, ordersRes, depositsRes] = await Promise.all([
      getSupabase().from('members').select('*').order('id'),
      getSupabase().from('shops').select('*').order('id'),
      getSupabase().from('menus').select('*').order('id'),
      getSupabase().from('orders').select('*').order('date'),
      getSupabase().from('deposits').select('*').order('id'),
    ]);
  ...
  const addMember = async (name, initialBalance = 0) => {
    const { data, error } = await getSupabase().from('members')
      .insert({ name, initial_balance: initialBalance }).select().single();
  ...
  const addShop = async (name, color = '#4a90d9') => {
    const { data, error } = await getSupabase().from('shops').insert({ name, color }).select().single();
    if (error) throw error;
    data.menus = [];
  ...
  const addMenu = async (shopId, name, price) => {
    const { data, error } = await getSupabase().from('menus')
      .insert({ shop_id: shopId, name, price: Number(price) }).select().single();
    if (error) throw error;
  ...
  const addOrder = async (date, memberId, shopId, menuName, price, note = '') => {
    const { data, error } = await getSupabase().from('orders')
      .insert({ date, member_id: memberId, shop_id: shopId, menu_name: menuName, price: Number(price), note })
      .select().single();
    if (error) throw error;
  ...
  const addDeposit = async (memberId, month, amount) => {
    const { data, error } = await getSupabase().from('deposits')
      .insert({ member_id: memberId, month, amount: Number(amount) }).select().single();
    if (error) throw error;
  ...
  const value = {
    members, shops, orders, deposits, loaded,
    init,
