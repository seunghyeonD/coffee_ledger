...
export function StoreProvider({ children }) {
  ...
  // === Members ===
  const addMember = async (name, initialBalance = 0, email = null) => {
    const { data, error } = await getSupabase().from('members')
      .insert({ name, initial_balance: initialBalance, email: email || null, company_id: companyId }).select().single();
    if (error) throw error;
    setMembers(prev => [...prev, data]);
    return data;
  };

  const updateMember = async (id, name, email = null) => {
    const { error } = await getSupabase().from('members').update({ name, email: email || null }).eq('id', id);
    if (error) throw error;
    setMembers(prev => prev.map(m => m.id === id ? { ...m, name, email: email || null } : m));
  };
  ...
  const addOrder = async (date, memberId, shopId, menuName, price, note = '') => {
    ...
    // 잔액 부족 알림 — 발송 여부는 서버가 유저별 임계값으로 판단
