...
export function AuthProvider({ children }) {
  ...
  // 기업 정보 수정 (입금계좌 등) — 서버 API 경유, 성공 시 로컬 상태 갱신
  const updateCompanyInfo = async (patch) => {
    if (!company) throw new Error('기업이 선택되지 않았습니다.');
    const res = await authFetch('/api/companies/update', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ companyId: company.id, bankAccount: patch.bank_account ?? null }),
    });
    if (!res.ok) throw new Error('기업 정보 수정에 실패했습니다.');
    setCompany(prev => ({ ...prev, ...patch }));
    setUserCompanies(prev => prev.map(c => (c.id === company.id ? { ...c, ...patch } : c)));
  };
  ...
  const value = {
    ...
    selectCompany, clearCompany, createCompany, joinCompany, updateCompanyInfo,
