...
export function AuthProvider({ children }) {
  ...
  const createCompany = async (name) => {
    ...
    const { error: ucError } = await supabase
      .from('user_companies')
      .insert({ user_id: user.id, company_id: companyData.id, role: 'master' });
  ...
  const joinCompany = async (inviteCode) => {
    ...
    const { error: joinError } = await supabase
      .from('user_companies')
      .insert({ user_id: user.id, company_id: companyData.id, role: 'user' });
  ...
  const getCompanyMembers = async () => {
    if (!company) return [];
    const res = await fetch(`/api/companies/members?companyId=${company.id}`);
  ...
  const removeMember = async (targetUserId) => {
    if (!company) throw new Error('기업이 선택되지 않았습니다.');
    const res = await fetch(`/api/companies/members?companyId=${company.id}&userId=${targetUserId}`, {
      method: 'DELETE',
    });
